...
// Then declare an array called GDP with as much size. Now read and print the Gross Domestic Products of the countries.
...
int main()
{
	...
	GDP=(long double*) malloc(sizeof(long double)* n); //long double do not supported here
	...
	for (i=0;i<n;++i)
	{
		...
		scanf("%Lf", &GDP[i]); //scanf("%d",&arr1[k]) ; 
	}
	// the following loop prints the GDP's of countries
	for (i=0;i<n;++i)
	printf("\nGDP of country number %d is %0.2Lf",i+1,GDP[i]); 
